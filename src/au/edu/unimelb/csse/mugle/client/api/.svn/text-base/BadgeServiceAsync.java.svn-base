...
/**
 * Developer async API for accessing the badges (achievements) system. This
 * allows each game to award the user with badges, as well as update the
 * progress of badges.
 */
public interface BadgeServiceAsync {
	 ...
	 * @param callback
	 *            Called once the operation finishes. On success, passes
	 *            information about that badge. Fails (BadgeError) if no badge
	 *            by that name.
	 */
	 ...
	 *            The name of the badge
	 * @param callback
	 *            Called once the operation finishes. Fails (BadgeError) if no
	 *            badge by that name.
	 */
	void setAchieved(String gameToken, String name, AsyncCallback<Void> callback);

	/**
	 * Increments the progress of the badge by the given amount. If the progress
	 * reaches the maximum for this badge, sets the badge to achieved. The
	 * progress is capped at the badge maximum.
	 * 
	 * @param gameToken
	 *            The secret token for the current game.
	 * @param name
	 *            The name of the badge
	 * @param amount
	 *            The amount to increment the badge by. Must not be negative.
	 * @param callback
	 *            Called once the operation finishes. On success, passes true if
	 *            the badge was achieved as a result. Fails (BadgeError) if no
	 *            badge by that name.
	 */
	 ...
	 * @param callback
	 *            Called once the operation finishes. On success, passes true if
	 *            the badge is achieved. Fails (BadgeError) if no badge by that
	 *            name.
	 */
	...
	/**
	 * Returns the progress of the current badge. Use getBadgeInfo to retrieve
	 * the maximum progress for the badge.
	 * 
	 * @param gameToken
	 *            The secret token for the current game.
	 * @param name
	 ...
	 *            progress of the badge. Fails (BadgeError) if no badge by that
	 *            name.
	 */
