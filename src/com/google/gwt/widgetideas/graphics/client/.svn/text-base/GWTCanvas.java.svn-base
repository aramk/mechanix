...
public class GWTCanvas extends Widget {
	...
	private final GradientFactory gradientFactoryImpl = (GradientFactory) GWT
			.create(GradientFactory.class);

	/*
	 * Impl Instance. Compiler should statify all the methods, so we do not end
	 * up with duplicate code for each canvas instance.
	 */
	 ...
	 *            the image to be drawn
	 * @param sourceX
	 *            the start X position in the source image
	 * @param sourceY
	 *            the start Y position in the source image
	 * @param sourceWidth
	 *            the width in the source image you want to sample
	 * @param sourceHeight
	 *            the height in the source image you want to sample
	 * @param destX
	 *            the start X position in the destination image
	 * @param destY
	 *            the start Y position in the destination image
	 * @param destWidth
	 *            the width of drawn image in the destination
	 * @param destHeight
	 *            the height of the drawn image in the destination
	 */
	...
	/**
	 * Draws an input image from a canvas to a specified position on the canvas.
	 * Size defaults to the default dimensions of the source canvas.
	 * 
	 * @param img
	 *            the canvas object to be drawn
	 * @param offsetX
	 *            x coord of the top left corner in the destination space
	 * @param offsetY
	 *            y coord of the top left corner in the destination space
	 */
	public void drawImage(GWTCanvas img, double offsetX, double offsetY) {
		drawImage(img, offsetX, offsetY, img.getCoordWidth(),
				img.getCoordHeight());
	}

	/**
	 * Draws an input image from a canvas at a given position on the canvas.
	...
	public void drawImage(GWTCanvas img, double offsetX, double offsetY,
		...
		impl.drawImage(img.getElement(), 0, 0, img.getCoordWidth(),
				img.getCoordHeight(), offsetX, offsetY, width, height);
	}

	/**
	 * Draws an input image from a canvas at a given position on the canvas.
	 * Resizes image according to specified width and height and samples from
	 * the specified sourceY and sourceX.
	 * 
	 * <p>
	 * We recommend that the pixel and coordinate spaces be the same to provide
	 * consistent positioning and scaling results
	 ...
	 *            the height of the drawn image in the destination
	 */
	public void drawImage(GWTCanvas img, double sourceX, double sourceY,
			double sourceWidth, double sourceHeight, double destX,
			double destY, double destWidth, double destHeight) {

		impl.drawImage(img.getElement(), sourceX, sourceY, sourceWidth,
	 ...
	 * BŽzier curve with control point <b>(cpx, cpy)</b>, and then adds the
	 ...
	 * Internet Explorer. We would more than welcome a patch from the community
	 * to get gradients working on IE.
	 * 
	 * @param grad
	 *            {@link CanvasGradient}
	 */
	 ...
	 * value explicitly, the canvas uses the <code>GWTCanvas.MITER</code> line
	 * join style.
	 * 
	 * @param lineJoin
	 */
	 ...
	 * line join is set to <code>GWTCanvas.MITER</code>, the canvas uses the
