...
public class ImageLoader {
	 ...
	 * invoked with an array of ImageElements corresponding to the original
	 * input array of url Strings once all the images report an onload event.
	 * 
	 * @param urls
	...
	/**
	 * Stores the ImageElement reference so that when all the images report an
	 * onload, we can return the array of all the ImageElements.
	 * 
	 * @param img
	 */
	...
	private void dispatchIfComplete() {
		if (callBack != null && isAllLoaded()) {
			callBack.onImagesLoaded((ImageElement[]) images
					.toArray(new ImageElement[0]));
