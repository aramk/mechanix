...
public class PathElement {
	...
	public static String arc(double x, double y, double radius,
		...
		// HTML 5 canvas spec treats an arc that subtends more than 360 degrees
		// as a
		// full circle. It does not specify where the path should end up
		// afterwards.
		...
		if (Math.abs(endAngle - startAngle) > fullArc) {
			...
			return arcImpl(x, y, radius, startAngle, endAngle, antiClockwise,
					canvas);
		}
	}
	...
	private static String arcImpl(double x, double y, double radius,
		...
		startY = (y + Math.sin(startAngle)
				* (ar - GWTCanvasImplIE6.subPixelFactorHalf));
		endX = (x + Math.cos(endAngle)
				* (ar - GWTCanvasImplIE6.subPixelFactorHalf));
		endY = (y + Math.sin(endAngle)
				* (ar - GWTCanvasImplIE6.subPixelFactorHalf));
